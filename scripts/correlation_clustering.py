...
for k, v in allele_frequency_first_time_point.items():
    ...
    if amount >= 2:
        not_rear_mut[amount].append(
            ("_".join(map(str, sorted({tp for tp, _ in v}))), name)
        )
...
plt.savefig(
    f"{str(results_folder)}/{patient.lower()}_correlation.pdf", bbox_inches="tight"
)
plt.close()
...
R = dendrogram(Z, p=p, no_plot=True)  # show only the last p merged
...
with plt.rc_context({"lines.linewidth": 4.3}):
    ...
    plt.savefig(
        f"{str(results_folder)}/{patient.lower()}_dendrogram.pdf", bbox_inches="tight"
    )
plt.close()
...
[
    idx
    for idx, name in enumerate(list(temp.values()))
    if name == "chr8_32685712_C_SNV_T" or name == "chr21_41473456_A_SNV_G"
]
...
first_group = {"mutation": [], "frequency": [], "point": []}
...
second_group = {"mutation": [], "frequency": [], "point": []}
...
third_group = {"mutation": [], "frequency": [], "point": []}
...
plt.figure(figsize=(12, 8))
ax = seaborn.lineplot(pandas.DataFrame.from_dict(first_group), x="point", y="frequency")
ax = seaborn.lineplot(
    pandas.DataFrame.from_dict(second_group), x="point", y="frequency"
)
ax = seaborn.lineplot(pandas.DataFrame.from_dict(third_group), x="point", y="frequency")
