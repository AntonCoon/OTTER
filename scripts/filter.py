...
if LEVEL in ("hard", "medium"):
    ...
    if LEVEL == "hard":
        ...
        for i in range(len(present_in_both_rep_paths)):
            for j in range(i + 1, len(present_in_both_rep_paths)):
                sp.run(
                    f"comm -1 -2 {present_in_both_rep_paths[i]} {present_in_both_rep_paths[j]} >> {cur_tmp_file}",
                    shell=True,
                    check=True,
                )

        present_in_at_least_2_path = (
            settings.TMP_FOLDER + "variants_in_at_least_2_sort_uniq.tsv"
        )
        sp.run(
            f"sort {cur_tmp_file} | uniq > {present_in_at_least_2_path}",
            shell=True,
            check=True,
        )
    ...
    for i in range(len(filenames)):
        for j in range(i + 1, len(filenames)):
            ...
            f2 = filenames[j]
            sp.run(
                f"scripts/print_present_in_two_vcfs {f1} {f2} >> {cur_tmp_file}",
                shell=True,
                check=True,
            )
...
sp.run(
    f"tail -n+2 {vcf_output} | sort --key=1,1 --key=2n >> {sorted_vcf}",
    check=True,
    shell=True,
)
